...
function gerow_custom_color() {
    ...
    // Button Hover Color
    $color_code3 = get_theme_mod( 'gerow_color_option3', '#0049DC' );
    wp_enqueue_style( 'gerow-custom', GEROW_THEME_CSS_DIR . 'gerow-custom.css', [] );
    if ( $color_code3 != '' ) {
        $custom_css = '';
        $custom_css .= "html:root { --tg-blue: " . $color_code3 . "}";
        wp_add_inline_style( 'gerow-custom', $custom_css );
    }

    // Lavender Color
    $color_code4 = get_theme_mod( 'gerow_color_option4', '#E6EEFF' );
    wp_enqueue_style( 'gerow-custom', GEROW_THEME_CSS_DIR . 'gerow-custom.css', [] );
    if ( $color_code4 != '' ) {
        $custom_css = '';
        $custom_css .= "html:root { --tg-lavender: " . $color_code4 . "}";
        wp_add_inline_style( 'gerow-custom', $custom_css );
    }
