...
$gerow_video_url = function_exists('get_field') ? get_field('fromate_style') : NULL;
...
<?php if (is_single()) : ?>
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('blog-post-item-two blog-details-wrap format-video'); ?>>

        <?php if (has_post_thumbnail()) : ?>
            <div class="blog-details-thumb position-relative">
                <?php the_post_thumbnail('full', ['class' => 'img-responsive']); ?>
                <?php if (!empty($gerow_video_url)) : ?>
                    <a href="<?php print esc_url($gerow_video_url); ?>" class="video--icon popup-video"><i class="fas fa-play"></i></a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
...
<?php else : ?>
    ...
    <div class="col-md-6 grid-item">
        <article id="post-<?php the_ID(); ?>" <?php post_class('blog-post-item-two format-video'); ?>>

            <?php if (has_post_thumbnail()) : ?>
                <div class="blog-post-thumb-two position-relative">
                    <?php the_post_thumbnail('full', ['class' => 'img-responsive']); ?>
                    <?php if (!empty($gerow_video_url)) : ?>
                        <a href="<?php print esc_url($gerow_video_url); ?>" class="video--icon popup-video"><i class="fas fa-play"></i></a>
                    <?php endif; ?>
