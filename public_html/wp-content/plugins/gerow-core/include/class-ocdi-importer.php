...
class GENIX_OCDI_Demo_Importer {
    ...
    public function __construct() {
        ...
        add_action( 'init', [$this, 'genix_ocdi_rewrite_flush'] );
    }

    public function import_files_config() {

		$home_prevs = array(
			'genix_demo_home1' => array(
				'title' => esc_html__( 'Home Default', 'genixcore' ),
				'page'  => esc_html__( 'home', 'genixcore' ),
				'screenshot' => plugins_url( 'assets/img/demo/home1.png', dirname(__FILE__) ),
				'preview_link' => 'https://themedox.com/gerow/',
			),
		);
        ...
        foreach ( $home_prevs as $key => $prev ) {
            ...
            $config[] = [
                ...
                'import_notice'                => esc_html__('After you import this demo, you will get a demo like Our Live Website', 'genixcore' ),
            ];
        }
    ...
    public function ocdi_after_import_setup( $selected_file ) {
        ...
        update_option( 'gerow_ocdi_importer_flash', true );
    }
    ...
    public function genix_ocdi_rewrite_flush() {

        if ( get_option( 'gerow_ocdi_importer_flash' ) == true ) {
            flush_rewrite_rules();
            delete_option( 'gerow_ocdi_importer_flash' );
        }
    }
}

new GENIX_OCDI_Demo_Importer;
